    ...
    runFlParserTest
        ...
        ( Prog
            ...
            , DefVar
                ...
                ( ExpListComp
                    (ExpVar (LowerVar "b"))
                    [Guard (ExpVar (LowerVar "c")), Gen (PatVar (LowerVar "d")) (ExpVar (LowerVar "e"))]
                )
            , DefVar
                (PatVar (LowerVar "a"))
                ( ExpCase
                    ...
                    , CaseCl (PatInt 1) [GuardCl (Just (ExpBool True)) (ExpInt 1), GuardCl (Just (ExpBool False)) (ExpInt 1)]
                    ]
                )
    ...
    runFlParserTest
        ...
        ( Prog
            [ DefVar
                ...
                ( ExpInfix
                    Minus
                    (ExpInfix Plus (ExpPrefix Neg (ExpInt 1)) (ExpInt 1))
                    (ExpInfix Div (ExpInfix Mul (ExpInt 1) (ExpInt 1)) (ExpInt 1))
                )
            ...
            , DefVar
                ...
                ( ExpInfix
                    Or
                    (ExpInfix And (ExpBool True) (ExpPrefix Not (ExpBool True)))
                    (ExpBool True)
                )
            , DefVar
                (PatVar (LowerVar "a"))
                ( ExpInfix
                    Index
                    (ExpGroup (ExpInfix Cons (ExpInt 1) (ExpInfix Concat (ExpList []) (ExpList []))))
                    (ExpInt 0)
                )
            ]
        )
    runFlParserTest
        ...
        ( Prog
            ...
            , DefAdt
                (UpperVar "T")
                [TyVar (LowerVar "a")]
                [Ctor (UpperVar "A") [TyVar (LowerVar "a")], Ctor (UpperVar "B") []]
            ...
            , DefFunc
                (LowerVar "f")
                [PatVar (LowerVar "a")]
                [GuardCl (Just (ExpBool True)) (ExpInt 1), GuardCl (Just (ExpBool False)) (ExpInt 1)]
